# Building Siro: Part 2 - SageMaker Serverless Deployment

Welcome back! In this guide, we'll deploy a SageMaker Serverless endpoint that will generate embeddings for Siro's knowledge base. This is a critical component - it converts text into numerical vectors that can be searched and compared.

## REMINDER - MAJOR TIP!!

There's a file `gameplan.md` in the project root that describes the entire Siro project to an AI Agent, so that you can ask questions and get help. There's also an identical `CLAUDE.md` and `AGENTS.md` file. If you need help, simply start your favorite AI Agent, and give it this instruction:
...
1. Note the endpoint name from Terraform output (should be `siro-embedding-endpoint`)
2. Edit `.env` in Cursor
3. Update this line:
   ```
   # Part 2 - SageMaker
   SAGEMAKER_ENDPOINT=siro-embedding-endpoint
   ```
...
aws sagemaker-runtime invoke-endpoint --endpoint-name siro-embedding-endpoint --content-type application/json --body fileb://vectorize_me.json --output json /dev/stdout
```
...
# Invoke the endpoint and output directly to console
aws sagemaker-runtime invoke-endpoint --endpoint-name "siro-embedding-endpoint" --content-type "application/json" --body "fileb://vectorize_me.json" --output json output.json
...
```bash
aws sagemaker describe-endpoint --endpoint-name siro-embedding-endpoint
```
Status should be "InService"

2. **Check CloudWatch logs**:
```bash
aws logs tail /aws/sagemaker/Endpoints/siro-embedding-endpoint --follow
```

3. **Verify the HuggingFace model ID**:
Check that the endpoint is configured with the correct model:
```bash
aws sagemaker describe-model --model-name siro-embedding-model --query 'PrimaryContainer.Environment'
```
...
aws cloudwatch get-metric-statistics --namespace "AWS/SageMaker" --metric-name "Invocations" --dimensions Name=EndpointName,Value=siro-embedding-endpoint --start-time $(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%S) --end-time $(date -u +%Y-%m-%dT%H:%M:%S) --period 300 --statistics Sum --region $(aws configure get region)
```
...
```bash
terraform import aws_sagemaker_endpoint.embedding_endpoint siro-embedding-endpoint
terraform apply
```

**Option 2: Delete and recreate**
```bash
aws sagemaker delete-endpoint --endpoint-name siro-embedding-endpoint
...
Your SageMaker endpoint is ready and waiting. Let's continue building Siro! 🎉
