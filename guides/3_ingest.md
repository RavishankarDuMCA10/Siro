# Building Siro: Part 3 - Ingestion Pipeline with S3 Vectors
...
4. Configure:
   - Bucket name: `siro-vectors-{your-account-id}` (replace with your actual account ID)
...
sagemaker_endpoint_name = "siro-embedding-endpoint"  # From Part 2
```
...
VECTOR_BUCKET=siro-vectors-YOUR_ACCOUNT_ID
...
curl -X POST $SIRO_API_ENDPOINT \
  -H "x-api-key: $SIRO_API_KEY" \
...
- Check CloudWatch logs: `aws logs tail /aws/lambda/siro-ingest`
...
In [Guide 4](4_researcher.md), we'll deploy the Siro Researcher Agent that uses this infrastructure to provide intelligent investment insights.
