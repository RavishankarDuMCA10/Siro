...
7. For **Policy name**, enter: `SiroRDSCustomPolicy`
8. For **Description**, enter: `RDS and Data API permissions for Siro project`
9. Click **Create policy**

### Add Required AWS Managed Policies

1. Still in IAM, click **User groups** in the left sidebar
2. Click on the `SiroAccess` group (created in Guide 1)
...
```bash
# Starting from the project root (siro directory)
...
2. Edit the `.env` file in your project root:
   ...
   - In Cursor's file explorer, click on the `.env` file in the siro directory
...
3. Add these lines with values from Terraform output:
   ```
   # Part 5 - Database
   AURORA_CLUSTER_ARN=arn:aws:rds:us-east-1:123456789012:cluster:siro-aurora-cluster
   AURORA_SECRET_ARN=arn:aws:secretsmanager:us-east-1:123456789012:secret:siro-aurora-credentials-xxxxx
   ```
...
```bash
aws rds describe-db-clusters --db-cluster-identifier siro-aurora-cluster
```
...
```bash
aws rds describe-db-clusters --db-cluster-identifier siro-aurora-cluster --query 'DBClusters[0].EnableHttpEndpoint'
```
...
aws secretsmanager list-secrets --query "SecretList[?contains(Name, 'siro-aurora-credentials')].Name"

# Then get the secret value (replace with actual name from above)
aws secretsmanager get-secret-value --secret-id siro-aurora-credentials-xxxxx --query SecretString --output text | jq .
```
