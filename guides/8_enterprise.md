...
Welcome to the final guide in the Siro Financial Advisor deployment series! In this guide, we'll transform our application into a production-ready, enterprise-grade system by implementing best practices for scalability, security, monitoring, guardrails, explainability, and observability.

By the end of this guide, your Siro Financial Advisor will be:
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:rds-db:*:*:cluster:siro-database"
      },
      {
        Effect = "Allow"
        Action = ["lambda:InvokeFunction"]
        Resource = [
          "arn:aws:lambda:*:*:function:siro-tagger",
          "arn:aws:lambda:*:*:function:siro-reporter",
          "arn:aws:lambda:*:*:function:siro-charter",
          "arn:aws:lambda:*:*:function:siro-retirement"
        ]
      }
    ]
  })
...
**To view your secrets:** Navigate to AWS Console → Secrets Manager → Select your region (us-east-1) → You'll see secrets like `siro-database-secret` containing your Aurora credentials
...
3. **Select metric** → Choose "Lambda" → "By Function Name" → Select your function (e.g., siro-api)
...
6. **Name your alarm** (e.g., "siro-api-errors") and create it
...
async def invoke_agent_with_retry(
    ...
    try:
        response = await lambda_client.invoke(
            FunctionName=f"siro-{agent_name}",
...
4. Create a new project called "siro-financial-advisor"
...
- Configures the appropriate service name (e.g., 'siro_planner_agent')
...
1. **Agent Traces**
   ...
   - Filter by service name: `siro_planner_agent`, `siro_reporter_agent`, etc.
...
1. **Check environment variables are set:**
   ...
   ```bash
   aws lambda get-function-configuration --function-name siro-planner | grep LANGFUSE
   ```

2. **Verify OPENAI_API_KEY is set** (required for export):

   ```bash
   aws lambda get-function-configuration --function-name siro-planner | grep OPENAI_API_KEY
   ```
