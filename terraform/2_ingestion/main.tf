...
resource "aws_s3_bucket" "vectors" {
  bucket = "siro-vectors-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "siro-ingest-lambda-policy"
...
resource "aws_api_gateway_rest_api" "api" {
  name        = "siro-api"
  description = "Siro Financial Planner API"
